...
//#if defined(NEWARC)
		...
		/* add by Herbert Roider
                 */
                if(cons[i].type==absAngle)
		{
			dx = L1_P2_x - L1_P1_x;
			dy = L1_P2_y - L1_P1_y;
			hyp1=_hypot(dx,dy);
			...
			temp = dx;// ist der cos
			temp2 = cos(M_PI-angleP);
                        //std::cout << " error winkel: " << (temp+temp2)*(temp+temp2) << std::endl;
			error += (temp+temp2)*(temp+temp2);
                        //double err = (temp+temp2)*(temp+temp2);
//
			dy=dy/hyp1;
			temp = dy;// ist der sin
			temp2 = sin(angleP-M_PI);
                        //std::cout << " error winkel: " << (temp2+temp)*(temp2+temp) << std::endl;
			error += (temp+temp2)*(temp+temp2);
//
